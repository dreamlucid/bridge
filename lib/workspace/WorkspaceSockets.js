...
class WorkspaceSockets {
  ...
  #tryStoreExistingSocket (id, refresh, ws) {
    ...
    if (!existingSocket) {
      // This is expected after server restarts - the client tries to reconnect
      // but the socket no longer exists. We'll create a new socket instead.
      logger.debug('Socket ID not found, will create new socket', id)
      return
    }
