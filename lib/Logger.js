...
class Logger {
  ...
  /**
   * Check if DEBUG environment variable matches the logger name
   * Supports patterns like: name, name*, *name, *name*
   * @private
   * @param { String } loggerName - The logger name to check
   * @returns { Boolean }
   */
  static #shouldLogDebug (loggerName) {
    const debugEnv = process.env.DEBUG
    if (!debugEnv) {
      // If DEBUG is not set, log all debug messages (default behavior)
      return true
    }
    ...
    for (const pattern of patterns) {
      ...
      if (pattern.startsWith('-')) {
        ...
        if (Logger.#matchPattern(loggerName, negPattern)) {
          return false
        }
        continue
      }
      ...
      if (Logger.#matchPattern(loggerName, pattern)) {
        return true
      }
    }
  ...
  debug (...args) {
    ...
    // Check DEBUG environment variable to filter by logger name
    if (!Logger.#shouldLogDebug(this.meta?.name)) return
