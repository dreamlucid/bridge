...
// Track registered workspaces to avoid duplicate registrations
const registeredWorkspaces = new Set()

function registerWebRTCSendCommand (workspace) {
  if (registeredWorkspaces.has(workspace.id)) return
  // Register the command via workspace.api.commands which is the SCommands instance
  // This command will execute in the main thread context
  try {
    // workspace.api.commands is the SCommands instance
    // Register command that doesn't return a value (no transaction handling)
    ...
    registeredWorkspaces.add(workspace.id)
