...
class WorkspaceRegistry extends EventEmitter {
   ...
   */
  add (workspace) {
    const existing = this._map.get(workspace.id)
    if (existing && existing !== workspace) {
      // Replacing a live workspace instance drops its SCommands handlers while
      // callers may still assume id-based registrations remain valid.
      logger.warn('Replacing existing workspace instance in registry', workspace.id)
    }
