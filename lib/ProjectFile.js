...
class ProjectFile {
  ...
  writeWorkspace (filePath, workspace) {
    ...
    // Get persistent state data and add workspace ID so it persists across restarts
    // The ID is stored as _id in state (temporary key) so we need to add it as 'id' (persistent)
    const persistentData = JSON.parse(workspace.state.toStaticString())
    persistentData.id = workspace.id
    archive.zip.file('state.json', JSON.stringify(persistentData), { binary: false })
    archive.finalize()
  }
