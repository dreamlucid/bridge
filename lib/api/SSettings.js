...
class SSettings extends DIBase {
  ...
  registerSetting (specification) {
    ...
    */
    ...
    /*
    Check if a setting with the same group and title already exists
    to prevent duplicate registrations
    */
    const existingSettings = this.props.Workspace.state.data._settings?.[specification.group] || []
    const alreadyExists = existingSettings.some(
      existing => existing.group === specification.group && existing.title === specification.title
    )

    if (alreadyExists) {
      // Setting already exists, skip registration
      return true
    }
